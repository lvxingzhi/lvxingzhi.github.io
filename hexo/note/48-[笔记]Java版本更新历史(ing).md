---
title: 【笔记】Java版本更新历史(ing)
date: 2016-05-02 19:30:05
tags:
categories:
- 笔记
---
...
该meta-annotation应用于目标为class的annotation类型上, __被此annotattion标注的class会自动继承父类的annotation__ 
...
**五, Lambda作用域**
在lambda表达式中访问外层作用域和老版本的匿名对象中的方式很相似。__你可以直接访问标记了final的外层局部变量,或者实例的字段以及静态变量。__  
...
```
__但是和匿名对象不同的是，这里的变量num可以不用声明为final，该代码同样正确：__
```text
int num = 1;  
